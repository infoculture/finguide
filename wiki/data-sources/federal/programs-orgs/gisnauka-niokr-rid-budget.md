...
| Тип доступа | Смешанный: веб-интерфейсы и реестры на **gisnauka.ru**; **[глобальный поиск](https://gisnauka.ru/global-search)** и внутренний JSON **`/api/egisu/base/search`**; выгрузки через **«Сформировать выгрузку»** |
...
Карточка описывает **публичный слой данных**, относящийся к **ГИС «Наука»** (`https://gisnauka.ru/`): учёт **НИОКТР**, регистрация и отчётность по **РИД**, сведения об организациях и проектах. На портале доступны **[глобальный поиск](https://gisnauka.ru/global-search)** с внутренним вызовом **`https://gisnauka.ru/api/egisu/base/search`** (JSON, без опубликованной спецификации) и сценарий **«Сформировать выгрузку»** для сохранения дампов по **году** и **типу данных**. Связка с **бюджетом** в машиночитаемом виде чаще всего достигается **комбинацией** этих источников с **[открытыми данными единого портала бюджетной системы](/data-sources/federal/budget-gov-ru-datasets)** и при необходимости **[единой площадкой мер финподдержки](/data-sources/federal/promote-budget-gov-ru)** для контекста отборов — а не одним универсальным файлом «всё в одном».
...
Задаётся **моментом генерации** для файлов, собранных через **«Сформировать выгрузку»**, и политикой обновления данных в ответах **API** и интерфейсе поиска; единого регламента для «всех таблиц ГИС „Наука“» нет. Фиксируйте **дату снимка** и параметры, с которыми получена выгрузка.
...
- Автоматический доступ к **корню gisnauka.ru** из скриптов может блокироваться из‑за **TLS** или политики сайта; для конвейеров предпочитайте **стабильные файлы**, сохранённые после **«Сформировать выгрузку»**, или воспроизводимые запросы к API из браузера.
