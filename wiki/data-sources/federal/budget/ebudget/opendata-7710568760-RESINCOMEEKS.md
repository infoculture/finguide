---
title: "Отчет об управлении остатками на едином казначейском счете (ф.0503197) Раздел 2. Результат распределения доходов от операций по управлению остатками средств на едином казначейском счете"
sidebar_label: "Отчет об управлении остатками на едином казначейском счет…"
tags:
  - data-source
  - federal
  - budget
...
slug: '/data-sources/federal/opendata-7710568760-RESINCOMEEKS'
source_url: "https://budget.gov.ru/epbs/opendata/7710568760-RESINCOMEEKS/meta.json"
...
description: "Отчет об управлении остатками на едином казначейском счете (ф.0503197) Раздел 2. Результат распределения доходов от операций по управлению остатками средств на едином казначейском счете — именованный открытый набор каталога budget.gov.ru (7710568760-RESINCOMEEKS); паспорт набора meta.json и файловые версии в контуре ГИИС «Электронный бюджет»."
...
# Отчет об управлении остатками на едином казначейском счете (ф.0503197) Раздел 2. Результат распределения доходов от операций по управлению остатками средств на едином казначейском счете
...
Коротко: **Отчет об управлении остатками на едином казначейском счете (ф.0503197) Раздел 2. Результат распределения доходов от операций по управлению остатками средств на едином казначейском счете** — отдельная запись каталога `list.json` с постоянным идентификатором **`7710568760-RESINCOMEEKS`**; для автоматизации начните с паспорта `meta.json` и сверьте поля с задачей анализа (план, факт, НСИ, отчётность и т.д.).
...
| --- | --- | --- |
| Карточка набора (витрина) | https://budget.gov.ru/epbs/faces/p/%D0%94%D0%B0%D0%BD%D0%BD%D1%8B%D0%B5%20%D0%B8%20%D1%81%D0%B5%D1%80%D0%B2%D0%B8%D1%81%D1%8B/opendata?code=7710568760-RESINCOMEEKS | HTML |
| Паспорт набора | https://budget.gov.ru/epbs/opendata/7710568760-RESINCOMEEKS/meta.json | JSON |
...
```bash
curl -sS -A "Mozilla/5.0" "https://budget.gov.ru/epbs/opendata/7710568760-RESINCOMEEKS/meta.json" | python3 -m json.tool | head -60
```
