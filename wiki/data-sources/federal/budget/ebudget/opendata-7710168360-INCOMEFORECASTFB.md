---
title: "Прогноз доходов федерального бюджета на очередной финансовый год, первый и второй год планового периода"
sidebar_label: "Прогноз доходов федерального бюджета на очередной финансо…"
tags:
  - data-source
  - federal
  - budget
...
slug: '/data-sources/federal/opendata-7710168360-INCOMEFORECASTFB'
source_url: "https://budget.gov.ru/epbs/opendata/7710168360-INCOMEFORECASTFB/meta.json"
...
description: "Прогноз доходов федерального бюджета на очередной финансовый год, первый и второй год планового периода — именованный открытый набор каталога budget.gov.ru (7710168360-INCOMEFORECASTFB); паспорт набора meta.json и файловые версии в контуре ГИИС «Электронный бюджет»."
...
# Прогноз доходов федерального бюджета на очередной финансовый год, первый и второй год планового периода
...
Коротко: **Прогноз доходов федерального бюджета на очередной финансовый год, первый и второй год планового периода** — отдельная запись каталога `list.json` с постоянным идентификатором **`7710168360-INCOMEFORECASTFB`**; для автоматизации начните с паспорта `meta.json` и сверьте поля с задачей анализа (план, факт, НСИ, отчётность и т.д.).
...
| --- | --- | --- |
| Карточка набора (витрина) | https://budget.gov.ru/epbs/faces/p/%D0%94%D0%B0%D0%BD%D0%BD%D1%8B%D0%B5%20%D0%B8%20%D1%81%D0%B5%D1%80%D0%B2%D0%B8%D1%81%D1%8B/opendata?code=7710168360-INCOMEFORECASTFB | HTML |
| Паспорт набора | https://budget.gov.ru/epbs/opendata/7710168360-INCOMEFORECASTFB/meta.json | JSON |
...
```bash
curl -sS -A "Mozilla/5.0" "https://budget.gov.ru/epbs/opendata/7710168360-INCOMEFORECASTFB/meta.json" | python3 -m json.tool | head -60
```
