---
title: "Перечень и объемы привлечения муниципальных внутренних заимствований на очередной финансовый год и плановый период"
sidebar_label: "Перечень и объемы привлечения муниципальных внутренних за…"
tags:
  - data-source
  - federal
  - budget
...
slug: '/data-sources/federal/opendata-7710168360-PROGINTERBORROWMO'
source_url: "https://budget.gov.ru/epbs/opendata/7710168360-PROGINTERBORROWMO/meta.json"
...
description: "Перечень и объемы привлечения муниципальных внутренних заимствований на очередной финансовый год и плановый период — именованный открытый набор каталога budget.gov.ru (7710168360-PROGINTERBORROWMO); паспорт набора meta.json и файловые версии в контуре ГИИС «Электронный бюджет»."
...
# Перечень и объемы привлечения муниципальных внутренних заимствований на очередной финансовый год и плановый период
...
Коротко: **Перечень и объемы привлечения муниципальных внутренних заимствований на очередной финансовый год и плановый период** — отдельная запись каталога `list.json` с постоянным идентификатором **`7710168360-PROGINTERBORROWMO`**; для автоматизации начните с паспорта `meta.json` и сверьте поля с задачей анализа (план, факт, НСИ, отчётность и т.д.).
...
| --- | --- | --- |
| Карточка набора (витрина) | https://budget.gov.ru/epbs/faces/p/%D0%94%D0%B0%D0%BD%D0%BD%D1%8B%D0%B5%20%D0%B8%20%D1%81%D0%B5%D1%80%D0%B2%D0%B8%D1%81%D1%8B/opendata?code=7710168360-PROGINTERBORROWMO | HTML |
| Паспорт набора | https://budget.gov.ru/epbs/opendata/7710168360-PROGINTERBORROWMO/meta.json | JSON |
...
```bash
curl -sS -A "Mozilla/5.0" "https://budget.gov.ru/epbs/opendata/7710168360-PROGINTERBORROWMO/meta.json" | python3 -m json.tool | head -60
```
