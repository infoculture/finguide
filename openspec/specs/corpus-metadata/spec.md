...
Для фактологических страниц и карточек источников шаблоны и документация SHALL поддерживать: `source_authority`, `rag_priority`, `related_pages`, `last_verified`.

Правила обязательности для подтипов SHOULD быть заданы в линтере или в [`AGENTS.md`](../../../AGENTS.md).

#### Scenario: Source template documents trust fields

- **GIVEN** автор создаёт карточку источника по шаблону
- **WHEN** он просматривает таблицу frontmatter
- **THEN** он видит назначение `rag_priority`, `related_pages`, `last_verified` и поля происхождения
...
Для повышения предсказуемости retrieval и редакционных выборок репозиторий SHALL поддерживать во frontmatter страниц под `wiki/` дополнительные поля, согласованные со спеками [`data-source-quality`](../data-source-quality/spec.md) и [`rag-corpus-navigation`](../rag-corpus-navigation/spec.md):
