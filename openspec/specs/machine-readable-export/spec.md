...
Машинно-читаемый слой для RAG и ассистентов: канонические точки входа, детерминированные документные артефакты, граф связей и QA-контракт.
## Requirements
...
#### Scenario: Consumer finds machine-readable entrypoints

- **GIVEN** новый потребитель открывает корень репозитория
- **WHEN** он читает `llms.txt` и `KNOWLEDGE.md`
- **THEN** он находит описание канонических экспортов и правил цитирования
...
Проект SHALL содержать [`tests/qa-validation.jsonl`](../../../tests/qa-validation.jsonl) с наборами вопросов, ожидаемых опорных slug и критериев качества для регрессии retrieval (целевой объём 50–100 записей).

#### Scenario: Retrieval regression has a versioned fixture

- **GIVEN** редактор добавляет новый крупный тематический пакет
- **WHEN** пакет готовится к merge
- **THEN** QA-корпус содержит хотя бы один вопрос с существующими каноническими slug по теме
...
Проект SHALL содержать страницу [`wiki/intro/rag-guide.md`](../../../wiki/intro/rag-guide.md) для разработчиков пайплайнов (чанкинг, приоритеты, метаданные).
...
#### Scenario: RAG guide explains draft filtering

- **GIVEN** разработчик строит retrieval по экспортам
- **WHEN** он читает RAG guide
- **THEN** он видит правила чанкинга, приоритетов и исключения `draft: true`
